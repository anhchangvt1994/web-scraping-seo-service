...
const workerManager = _WorkerManager2.default.init(
	_path2.default.resolve(__dirname, `./FollowResource.worker/index.${_constants.resourceExtension}`),
	{
		minWorkers: 1,
		maxWorkers: 4,
	},
...
const CleanerService = async () => {
	...
	const cleanBrowsers = (() => {
		let executablePath
		return async (durationValidToKeep = _InitEnv.PROCESS_ENV.RESET_RESOURCE ? 0 : 1) => {
			const browserStore = (() => {
	...
	const cleanPages = async (
		...
		try {
			await pool.exec('scanToCleanPages', [_constants.pagesPath, durationValidToKeep])
		} catch (err) {
			_ConsoleHandler2.default.error(err)
		} finally {
			freePool.terminate()
