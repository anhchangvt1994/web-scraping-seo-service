...
// 	/<script(>|\s(?![\s\S]*only-dev)[\s\S]*?(\/>|>))[\s\S]*?<\/script>(?:[\s\S]*?|$)|<style(>|\s(?![\s\S]*only-dev)[\s\S]*?(\/>|>))[\s\S]*?<\/style>(?:[\s\S]*?|$)|<link[\s\S]*href="[\s\S]*\.(css)+(|\?v=.*)"[\s\S]*?(\/|)>(?:[\s\S]*?|$)|(\s+|)style="([\s\S].+|)"/g
 const regexOptimizeForScriptBlockPerformance =
	/(<script(\s[^>]+)*>(.|[\r\n])*?<\/script>|<script(\s[^>]+)*\/>)/g; exports.regexOptimizeForScriptBlockPerformance = regexOptimizeForScriptBlockPerformance
 const regexOptimizeForPerformanceNormally =
	/(<link\s+(?=.*(rel=["']?(modulepreload|preload|prefetch)["']?).*?(\/|)?)(?:.*?\/?>))|<iframe\s+(?:[^>]*?\s+)?((src|id)=["']?[^"]*\b((partytown|insider-worker)(?:-[a-z]+)?)\b[^"]*["']|\bvideo\b)?[^>]*>(?:[^<]*|<(?!\/iframe>))*<\/iframe>/g; exports.regexOptimizeForPerformanceNormally = regexOptimizeForPerformanceNormally
 const regexOptimizeForPerformanceHardly =
	/(<style(\s[^>]+)*>(.|[\r\n])*?<\/style>|<style(\s[^>]+)*\/>|<link\s+(?=.*(rel=["']?(stylesheet|shortcut icon)["']?|href=["']?.*?(css|style).*?["']?).*?(\/|)?)(?:.*?\/?>))|<video(?![\s\S]*seo-tag)(\s[^>]+)*>(.|[\r\n])*?<\/video>|<audio(?![\s\S]*seo-tag)(\s[^>]+)*>(.|[\r\n])*?<\/audio>|<(video|audio)(?![\s\S]*seo-tag)(\s[^>]+)*\/>|<form(\s[^>]+)*>(.|[\r\n])*?<\/form>|<input(?![^>]*\b(?:type=['"](?:button|submit)['"]|type=(?:button|submit)\b)[^>]*>)[^>]*>|<textarea(\s[^>]+)*\/>|<textarea(\s[^>]+)*>(.|[\r\n])*?<\/textarea>|<label\s+(?=.*(for=["']?.*?["']?).*?(\/|)?)(?:.*?\/?>)|<svg(\s[^>]+)*>(.|[\r\n])*?<\/svg>|<span\s+(?:[^>]*?\s+)?class=["']?[^"]*\b((fa-|material-icons|icon(-\w*)*|ri-)(?:-[a-z]+)?)\b[^"]*["']?[^>]*>(?:[^<]*|<(?!\/span>))*<\/span>|<i\s+(?:[^>]*?\s+)?class=["']?[^"]*\b((fa-|material-icons|icon(-\w*)*|ri-)(?:-[a-z]+)?)\b[^"]*["']?[^>]*>(?:[^<]*|<(?!\/i>))*<\/i>|<img\s+(?=.*class=["']?.*?\b((fa-|material-icons|icon(-\w*)*|ri-)(?:-[a-z]+)?)\b.*?["']?.*?(\/|)?)(?:.*?\/?>)|<img\s+(?=.*alt=["']?.*?\b(icon(-\w*)*(?:-[a-z]+)?)\b.*?["']?.*?(\/|)?)(?:.*?\/?>)|style=(?:("|'|)([^"']+)("|'|)[^>\s]*)|class=(?:("|'|)([^"']+)("|'|)[^>\s]*)|(<div(>|[\s\S]*?(>))|<\/div>)(?:[\s\S]*?|$)/g; exports.regexOptimizeForPerformanceHardly = regexOptimizeForPerformanceHardly
...
// 	/<(a|button|input)(?![^>]*rel="nofollow")([^>]*)(\/|)>([\s\S]*?)<\/(a|button)>/g
 ...
 const regexQueryStringSpecialInfo =
	/botInfo=(?<botInfo>[^&]*)&deviceInfo=(?<deviceInfo>[^&]*)/; exports.regexQueryStringSpecialInfo = regexQueryStringSpecialInfo
 ...
 const DISABLE_COMPRESS_HTML = !!process.env.DISABLE_COMPRESS_HTML; exports.DISABLE_COMPRESS_HTML = DISABLE_COMPRESS_HTML
 const DISABLE_DEEP_OPTIMIZE = !!process.env.DISABLE_DEEP_OPTIMIZE; exports.DISABLE_DEEP_OPTIMIZE = DISABLE_DEEP_OPTIMIZE
 const DISABLE_OPTIMIZE = !!process.env.DISABLE_OPTIMIZE; exports.DISABLE_OPTIMIZE = DISABLE_OPTIMIZE

 const NOT_FOUND_PAGE_ID = process.env.NOT_FOUND_PAGE_ID || '404-page'; exports.NOT_FOUND_PAGE_ID = NOT_FOUND_PAGE_ID
 ...
 const chromiumPath =
	'https://github.com/Sparticuz/chromium/releases/download/v119.0.2/chromium-v119.0.2-pack.tar'; exports.chromiumPath = chromiumPath

 const canUseLinuxChromium =
	_constants.serverInfo &&
	_constants.serverInfo.isServer &&
	_constants.serverInfo.platform.toLowerCase() === 'linux'; exports.canUseLinuxChromium = canUseLinuxChromium
 ...
 const DISABLE_SSR_CACHE = Boolean(process.env.DISABLE_SSR_CACHE); exports.DISABLE_SSR_CACHE = DISABLE_SSR_CACHE
