...
exports. default = (() => {
  const litmitEmptyContentDuration = _InitProcessEnv2.default.BROWSERLESS_API_KEY
    ? 1800000
    : 150000;
  ...
  const _init = ({
      ...
      if (path) {
        try {
          await _optionalChain([_workerpool2.default, 'access', _ => _.pool, 'call', _2 => _2(
            __dirname + `./FollowResource.worker/index.${_constants.resourceExtension}`
      ...
      const clean = async (
        ...
        setTimeout(async () => {
          ...
          try {
            ...
            _optionalChain([fail, 'optionalCall', _5 => _5(result, () => clean(file, options))]);
            return result;
          } catch (err) {
            _ConsoleHandler2.default.error(err);
            followFileWithSchedulePool.terminate();
            _optionalChain([fail, 'optionalCall', _6 => _6(false, () => clean(file, options))]);
            return false;
          }
        }, schedule);
      }; // clean
    ...
    const browserInit = (() => {
      ...
      const _newPage = async () => {
        ...
        try {
          ...
          page = await _optionalChain([browser, 'optionalAccess', _7 => _7.newPage, 'optionalCall', _8 => _8()]);
        } catch (err) {
          return;
        }
    ...
    const handleRequest = async (
      ...
      const page = await _optionalChain([browserInit, 'optionalAccess', _9 => _9.newPage, 'optionalCall', _10 => _10()]);
      ...
      try {
        ...
        const specialInfo = _nullishCoalesce(_optionalChain([_constants3.regexQueryStringSpecialInfo, 'access', _11 => _11.exec, 'call', _12 => _12(url), 'optionalAccess', _13 => _13.groups]), () => ( {}));
        ...
        isCompleteGetHtmlProcess = await new Promise(async (res) => {
          ...
          status = _optionalChain([response, 'optionalAccess', _14 => _14.status, 'call', _15 => _15()]);
      ...
      _ConsoleHandler2.default.info(`Headless rendered page in: ${ttRenderMs}ms`);
    ...
    const SSRGenerator = async (url) => {
      ...
      if (result && !result.available) {
        ...
        _ConsoleHandler2.default.log("Kiểm tra có đủ điều kiện tạo page mới không ?");
        ...
        if (result) {
          ...
          if (result.isInit) {
            ...
            cache.clean(result.file, {
              schedule: _InitProcessEnv2.default.BROWSERLESS_API_KEY ? 1800000 : 300000,
