...
const ISRHandler = async (params) => {
	...
	if (_constants3.CACHEABLE_STATUS_CODE[status]) {
		...
		const crawlCustomOption = _optionalChain([_serverconfig2.default, 'access', _61 => _61.crawl, 'access', _62 => _62.custom, 'optionalCall', _63 => _63(url)])
		...
		let isRaw = false
		try {
			if (enableScriptOptimize) html = await _OptimizeHtmlworker.scriptOptimizeContent.call(void 0, html)

			if (enableStyleOptimize) html = await _OptimizeHtmlworker.styleOptimizeContent.call(void 0, html)

			if (enableShallowOptimize) html = await _OptimizeHtmlworker.shallowOptimizeContent.call(void 0, html)

			if (enableToCompress) html = await _utils3.compressContent.call(void 0, html)

			if (enableDeepOptimize) html = await _OptimizeHtmlworker.deepOptimizeContent.call(void 0, html)
